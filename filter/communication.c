    ...
    *connectionCookie = connectionCtx->Type;
    switch (connectionCtx->Type) {
        ...
        default:
            ...
            AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV]: AvConnectNotifyCallback: No such connection type. \n"));
    ...
    AV_DBG_PRINTEX((0, AVDBG_TRACE_DEBUG, "[AV]: AvConnectNotifyCallback entered. type: %d \n", connectionCtx->Type));
...
--*/
{
    ...
    switch (*connectionType) {
        ...
        default:
            ...
            AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV]: AvDisconnectNotifyCallback: No such connection type. \n"));
            return;
    }
    ...
    AV_DBG_PRINTEX((0, AVDBG_TRACE_DEBUG, "[AV]: AvDisconnectNotifyCallback entered. type: %d \n", *connectionType));
...
--*/
{
    ...
    AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV] AvGetScanCtxSynchronized: scan context not found. \n"));
    *ScanCtx = NULL;
...
--*/
{
    ...
    if ( !NT_SUCCESS(status) ) {
        ...
        AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV] AvGetInstanceContextByVolume: Failed to enumerate instances. \n"));
        return status;
    }
    ...
    for (i = 0; i < instCnt; i++) {
        ...
        if ( !NT_SUCCESS(status) ) {
            ...
            AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV] AvGetInstanceContextByVolume: Failed to get instance context. \n"));
            break;
        }
    ...
    AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV] AvGetInstanceContextByVolume: instance context not found. \n"));
    if ( NT_SUCCESS( status ) ){
...
--*/
{
    ...
    if (!NT_SUCCESS(status)) {
        ...
        AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV] AvGetInstanceContextByFileHandle: Failed to get file object by handle. \n"));
        return status;
    }
    ...
    try {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV] AvGetInstanceContextByFileHandle: Failed to get volume by file object. \n"));
            leave;
        }
...
--*/
{
    ...
    if (!NT_SUCCESS(status)) {
        ...
        AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV]: ***AvGetInstanceContextByFileHandle FAILED. \n"));
        return status;
    }
    try {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV] AvGetStreamContextByHandle: Failed to get file object by handle. \n"));
            leave;
        }
...
--*/
{
    ...
    if (!NT_SUCCESS( status )) {
        ...
        AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV] AvHandleCmdCreateSectionForDataScan: failed to get stream context.\n"));
        goto Cleanup;
    }
    ...
    if (!NT_SUCCESS( status )) {
        ...
        AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV] AvHandleCmdCreateSectionForDataScan: failed to create section context.\n"));
        goto Cleanup;
    }
    ...
    if (ScanContext->IoWaitOnScanCompleteNotificationAborted) {
        ...
        AV_DBG_PRINTEX((0, AVDBG_TRACE_DEBUG, "[AV] AvHandleCmdCreateSectionForDataScan: Before FltCreateSectionForDataScan, it found Io is trying to abort the wait.\n"));
        goto Cleanup;
    }
    ...
    if (!NT_SUCCESS( status )) {
        ...
        if (NT_SUCCESS( sta )) {
            ...
            if (NT_SUCCESS( sta )) {
                ...
                AV_DBG_PRINTEX((0, AVDBG_TRACE_DEBUG, "[AV] ############## %wZ, %wZ, %wZ\n",
                    property->FileSystemDriverName,
                    property->FileSystemDeviceName,
                    property->RealDeviceName));
            }
        ...
        AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV] AvHandleCmdCreateSectionForDataScan: %I64x,%I64x failed to create section object. 0x%x\n",
            streamContext->FileId.FileId64.UpperZeroes,
            streamContext->FileId.FileId64.Value,
            sta));
#endif  // DBG
...
Cleanup:
    ...
    if (NT_SUCCESS( status ) &&
        ...
        AV_DBG_PRINTEX((0, AVDBG_TRACE_DEBUG, "[AV] AvHandleCmdCreateSectionForDataScan: After FltCreateSectionForDataScan, it found Io is trying to abort the wait.\n"));
        //
...
--*/
{
    ...
    switch( ScanResult ) {
        ...
        case AvScanResultUndetermined:
            ...
            AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "***[AV] AvUpdateScanResult: the caller did not specify the scan result.\n"));
            //
...

{
    ...
    if (!NT_SUCCESS(status)) {
        ...
        AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "***[AV]: AvFinalizeSectionContext: Close section failed.\n"));
    }
    FltReleaseContext( SectionContext );
    return status;
}
...
--*/
{
    ...
    if (!NT_SUCCESS( status )) {
        ...
        AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "***[AV] AvHandleCmdCloseSectionForDataScan: failed to get stream context.\n"));
        goto Cleanup;
    }
...
--*/
{
    ...
    strncpy(OutputBuffer, "Hi user", 7);
    AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV]: AvMessageNotifyCallback entered. sending %s\n", OutputBuffer));

    if ((InputBuffer == NULL) ||
        (InputBufferSize < (FIELD_OFFSET(COMMAND_MESSAGE, Command) +
                            sizeof(AVSCAN_COMMAND)))) {

        AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] 0x2000001\n"));
    ...
    try {
        ...
        AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "User sent: %d:%s\n", AVDBG_TRACE_ZERO, (PCHAR)InputBuffer));

    } except (AvExceptionFilter( GetExceptionInformation(), TRUE )) {

        AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] 0x2000002\n"));
    ...
    switch (command) {
        ...
        case AvCmdCreateSectionForDataScan:
            ...
            AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] AvCmdCreateSectionForDataScan called\n"));
            if ((OutputBufferSize < sizeof (HANDLE)) ||
                    (OutputBuffer == NULL)) {

                AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] 0x2000003\n"));
                return STATUS_INVALID_PARAMETER;
            }

            if (!IS_ALIGNED(OutputBuffer,sizeof(HANDLE))) {

                AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] 0x2000004\n"));
            ...
            if (!NT_SUCCESS( status )) {
                ...
                AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] 0x2000005\n"));
        ...
        case AvCmdCloseSectionForDataScan:
            ...
            try {
                ...
                if (scanResult == AvScanResultInfected) {
                    AV_DBG_PRINTEX((0, AVDBG_TRACE_OPERATION_STATUS, "[AV]: *******AvCmdCreateSectionForDataScan FAILED. \n"));
                }

            } except (AvExceptionFilter( GetExceptionInformation(), TRUE )) {

                AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] 0x2000006\n"));
                return GetExceptionCode();
            }
            ...
            if (!NT_SUCCESS( status )) {
                ...
                AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] 0x2000007\n"));
                return STATUS_NOT_FOUND;
            }
        ...
        case AvIsFileModified:
            ...
            try {
                ...
                AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] 0x2000008\n"));
                return GetExceptionCode();
            }
            ...
            if ((OutputBufferSize < sizeof (BOOLEAN)) ||
                ...
                AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] 0x2000009\n"));
                return STATUS_INVALID_PARAMETER;
            }

            if (!IS_ALIGNED(OutputBuffer,sizeof(BOOLEAN))) {

                AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] 0x200000a\n"));
                return STATUS_DATATYPE_MISALIGNMENT;
            }
            ...
            if (!NT_SUCCESS(status)) {
                AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR, "[AV]: **************************AvGetStreamContextByHandle FAILED. \n"));
                break;
            }
        ...
        default:
            ...
            AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] 0x200000b\n"));
            return STATUS_SUCCESS;
            //return STATUS_INVALID_PARAMETER;
    }

    AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] 0x0000000\n"));
    return status;
...
--*/
{
    ...
    AV_DBG_PRINTEX((0, AVDBG_TRACE_DEBUG, "[AV]: AvPrepareServerPort entered. \n"));
    ...
    DbgPrintEx(0,0, "[AV] port name is: %ls\r\n", portName);
