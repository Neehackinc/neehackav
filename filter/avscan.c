...
NTSTATUS sendMessageToUserMode(IN OUT PUNICODE_STRING pFullPathName);
...
--*/
{
    ...
    if (!NT_SUCCESS( status )) {
        ...
        AV_DBG_PRINT( AVDBG_TRACE_ERROR, ("[AV] AvInstanceSetup: allocate instance context failed. status = 0x%x\n", status) );
...
--*/
{
    ...
    DbgPrintEx(0, 0, "[AV] Driver Entry: Entered\r\n");
    ...
    Globals.DebugLevel = 0xffffffff; // AVDBG_TRACE_ERROR | AVDBG_TRACE_DEBUG; set the value to 0x00000000 to disable debugger prints
    ...
    try {
        ...
        status = AvSetConfiguration( RegistryPath );
        ...
        if (!NT_SUCCESS( status )) {
            ...
            DbgPrintEx(0, 0, "[AV] DriverEntry: AvPrepareServerPort Scan Port FAILED. status = 0x%x\n", status);
            leave;
        }
        ...
        if (!NT_SUCCESS( status )) {
            ...
            DbgPrintEx(0, 0, "[AV] DriverEntry: AvPrepareServerPort Abort Port FAILED. status = 0x%x\n", status);
            leave;
        }
        ...
        if (!NT_SUCCESS( status )) {
            ...
            DbgPrintEx(0, 0, "[AV] DriverEntry: AvPrepareServerPort Query Port FAILED. status = 0x%x\n", status);
            leave;
        }
...
--*/
{
    ...
    if (!NT_SUCCESS( status ) ||
        ...
        AV_DBG_PRINTEX( (AVDBG_TRACE_ROUTINES,0, "[AV] AvPostCreate: file creation failed\n") );
    ...
    if (!(FlagOn(desiredAccess, FILE_WRITE_DATA)) && 
        ...
        if (!NT_SUCCESS( status )) {
            ...
            AV_DBG_PRINTEX(( 0, AVDBG_TRACE_ROUTINES, "[AV] AvPostCreate: AvGetFileEncrypted FAILED!! \n0x%x\n", status) );
        }
        if (encrypted) {
            return FLT_POSTOP_FINISHED_PROCESSING;
        }
    ...
    */
    ...
    //AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV]: filtsendmessage: 0x%08x, %wZ\n", status, nameInfo->Name));
    /*
    DbgPrintEx(0, 0, "[Av]: Full path: %wZ, Dir: %wZ, FinalComponent: %wZ, Stream: %wZ, sLen: %d\n",
        nameInfo->Name,
        nameInfo->ParentDir,
        nameInfo->FinalComponent,
        nameInfo->Stream,
    ...
    */
    ...
    if (status == STATUS_NOT_FOUND) {
        ...
        if (!NT_SUCCESS( status )) {
            ...
            AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO,
        ...
        if (!NT_SUCCESS( status )) {
            ...
            AV_DBG_PRINTEX( (AVDBG_TRACE_ROUTINES,0,
        ...
        if (!NT_SUCCESS(status)) {
            ...
            if (status == STATUS_FLT_CONTEXT_ALREADY_DEFINED) {
                ...
                AV_DBG_PRINTEX( (AVDBG_TRACE_ERROR, 0, 
        ...
        AV_DBG_PRINTEX(( 0, AVDBG_TRACE_ERROR,
    ...
    PUNICODE_STRING filePath = GetFullPath(nameInfo, Data, FltObjects, &nameInfo->Name);

    if (Data->IoStatus.Information == FILE_OPENED) {
        AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "FileOpened: %wZ\n", filePath));
        if (Globals.ScanClientPort == NULL) {
            AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "userland not available\n", filePath));
        }
        else {
            status = sendMessageToUserMode(filePath);
            if (status == STATUS_FILE_INVALID) {

                AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "STATUS_FILE_INVALID sent\n"));
                //Data->IoStatus.Status = STATUS_ACCESS_DENIED;//STATUS_FILE_INVALID;
    ...
    if (FlagOn( Data->Iopb->Parameters.Create.SecurityContext->DesiredAccess,
        ...
        //
        AV_DBG_PRINTEX((0, AVDBG_TRACE_ERROR,"[AV] AvPostCreate: Data->Iopb->Parameters.Create.SecurityContext->DesiredAccess is TRUE \n"));
    ...
    if (IS_FILE_NEED_SCAN( streamContext )) {
        ...
        if (!NT_SUCCESS( status ) ||
            ...
            AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "[AV] AvPostCreate: AvScan FAILED 0x1!! \n") );
...
GetFullPath(
    ...
    __try{
        ...
        pFullPathName->Buffer = ExAllocatePool2(POOL_FLAG_PAGED, 512, '3331');
        ...
        //ULONG replySize = sizeof(int);
        //status = FltSendMessage(Globals.Filter, &Globals.ScanClientPort, pFullPathName->Buffer, pFullPathName->Length, &replyFlag, &replySize, &timeOut);
        bResult = TRUE;
    }
...
NTSTATUS sendMessageToUserMode(IN OUT PUNICODE_STRING pFullPathName) {
    NTSTATUS status;
    ULONG replySize = 1024;
    REPLY_MESSAGE_STRUCT msgReply;

    LARGE_INTEGER timeOut = { 0 };
    timeOut.QuadPart = -500000000;
    RtlZeroMemory(msgReply.data, 1024);

    status = FltSendMessage(Globals.Filter, &Globals.ScanClientPort, pFullPathName->Buffer, pFullPathName->Length, &msgReply.data, &replySize, &timeOut);

    if (wcsstr(msgReply.data, L"malware detected")) {
        AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "malware detected\n"));
        return STATUS_FILE_INVALID;
    } else {
        AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "File is not infected\n"));
    }
    //AV_DBG_PRINTEX((0, AVDBG_TRACE_ZERO, "<Fullpath> [FullPathName: %wZ], [FullPathName Length: %d], [replySize: %ul], [replyMessage address: %p], [data: %ls]\n",
    //    pFullPathName, pFullPathName->Length, replySize, &replyMessage, msgReply.data));
    return STATUS_SUCCESS;
}
